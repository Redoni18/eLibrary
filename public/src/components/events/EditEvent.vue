                    ...
                    <div class="card-block">
                        <div class="row">
                            <div class="col-md-10 offset-1">
                                ...
                                <form method="post" @submit.prevent="editEvent">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async fetchEvent(){
                const response = await axios.get(`http://localhost:8000/api/event/${this.event_id}`)
                console.log(response.data);

                this.eventName = response.data.name
                this.eventDescription = response.data.description
                this.selectedBooks = response.data.books
            ...
            async editEvent() {
                this.$validator.validateAll().then( async (result) => {
                    if (result) {
                        await axios.put(`http://localhost:8000/api/editEvent/${this.event_id}`, {_id: this.event_id, name: this.eventName, description: this.eventDescription, books: this.selectedBooks, startDate: this.startDate, endDate: this.endDate})
                        this.eventName = null
