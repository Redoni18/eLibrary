...
<div>
    <div class="form-group row" v-if="toggleButtons">
        <div class="col-sm-10 offset-sm-0 offset-md-0 offset-md-9">
         <router-link :to="{name: 'EditTopic', params: {id: rowId}}"> <button type="submit" class="btn btn-primary">Edit Topic</button></router-link>
    ...
    <div class="mb-3">
        <router-link type="submit" class="btn btn-primary" :to="{path: 'insertTopic'}">Insert new Topic</router-link>
    ...
    <template v-if="user.data.isAdmin" v-slot:table-row="props">
      <span v-if="props.column.field === 'actions'">
        <mdb-dropdown end tag="li" class="nav-item">
            ...
            <mdb-dropdown-menu>
                <mdb-dropdown-item @click.native="showModal = true;topic=props.row._id;"><mdb-icon icon="trash" class="mr-3" />Delete</mdb-dropdown-item>
                <mdb-dropdown-item :to="{name: 'EditTopic', params: {id: props.row._id}}"><mdb-icon icon="pen" class="mr-3" />Edit</mdb-dropdown-item>
            </mdb-dropdown-menu>
        </mdb-dropdown>
      </span>
    </template>
...
<div>
    <mdb-modal centered v-if="showModal" @close="showModal = false">
      ...
      <mdb-modal-body>Are you sure you want to delete selected topic?</mdb-modal-body>
      <mdb-modal-footer>
        <mdb-btn color="primary" @click.native="showModal = false">Close</mdb-btn>
        <mdb-btn color="danger" @click.native="removeTopic(topic)">Delete</mdb-btn>
      </mdb-modal-footer>
    </mdb-modal>
  </div>
</div>
...
export default {
    ...
    methods: {
        ...
        async removeTopic(id) {
            await axios.delete(`http://localhost:8000/api/topic/delete/${id}`)
