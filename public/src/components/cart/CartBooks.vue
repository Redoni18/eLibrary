<template>
    <div class="users_container container">
        <div class="mb-3" v-if="userCartedBooks.length">
            <router-link type="submit" class="btn btn-primary" :to="{path: '/cartBooks/listing'}">Switch to table view</router-link>
        </div>
        <div class="row" v-if="userCartedBooks.length">
                
                <book class="single-book" @changePage="changePage(book._id)" v-for="book in userCartedBooks" :key="book._id" :title="book.title" :description="book.description" :author="book.author" :year="book.year" :imageUrl="book.imageUrl">
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                userCartedBooks: [] //userFavouritedBooks
            }
        },
        computed: {
            ...mapGetters({
